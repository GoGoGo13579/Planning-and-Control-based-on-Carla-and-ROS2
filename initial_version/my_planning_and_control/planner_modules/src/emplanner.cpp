...
{   _current_time = this->now().seconds();
    ...
    {
        path_index2s.emplace_back(path_index2s.back() + std::hypot(path_trajectory[i].x - path_trajectory[i-1].x,
                                                                  path_trajectory[i].y - path_trajectory[i-1].y));
    }
    ...
    {
        cartesion_set_to_frenet_set(_dynamic_obstacles, path_trajectory, path_trajectory.front(),dynamic_obstacle_frenet_coordinate);
    }
    //---------------测试
    ...
    if (_plot_count % 20 == 0)
    {
        ...
        // std::vector<double> final_qp_speed_profile_t_set, final_qp_speed_profile_s_set;
        // for (auto &&st_point : final_qp_speed_profile)
        // {
        //     final_qp_speed_profile_t_set.emplace_back(st_point.t);
        //     final_qp_speed_profile_s_set.emplace_back(st_point.s);
        // }
        // matplot::plot(final_qp_speed_profile_t_set, final_qp_speed_profile_s_set, "r*-")->line_width(2);
    ...
    // RCLCPP_INFO(this->get_logger(),"当前规划起点的sl坐标(%.3f,%.3f)",planning_start_point_frenet_coordinate.s,planning_start_point_frenet_coordinate.l);
    // for (auto &&min : final_dp_path_l_min)
    // {
    //     std::cout << min << ", " ;
    // }
    // std::cout << std::endl;
